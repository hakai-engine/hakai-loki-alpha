...
func (acc *Account) CreateSession(ctx context.Context, db *sql.DB) (string, error) {
	...
	if _, err = tx.ExecContext(
		writeCtx,
		"DELETE FROM `account_sessions` WHERE `expires` <= ?",
		now.Unix(),
	); err != nil {
		return "", sessionStorageError(err)
	}
...
func sessionStorageError(err error) error {
...
func sessionCreateError(err error) error {
	return serviceerrors.LoginService(
		serviceerrors.CodeSessionCreateFailed,
		"SESSION_CREATE_FAILED",
		err,
