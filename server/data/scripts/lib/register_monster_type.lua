...
function readSpell(incomingLua, mtype)
	...
	if incomingLua.castText and spell.setCastText then
		spell:setCastText(incomingLua.castText)
	end
	if incomingLua.pokemonMoveId and spell.setPokemonMoveId then
		spell:setPokemonMoveId(incomingLua.pokemonMoveId)
	end
	if incomingLua.pokemonType or incomingLua.pokemonCategory then
		if not incomingLua.pokemonType or not incomingLua.pokemonCategory then
			logger.error(
				"[readSpell] - Monster {}: Pokemon combat requires both pokemonType and pokemonCategory.",
				mtype:name()
			)
			return nil
		end
		if not spell.setPokemonCombat or not spell:setPokemonCombat(incomingLua.pokemonType, incomingLua.pokemonCategory, incomingLua.pokemonPower) then
			logger.error(
				"[readSpell] - Monster {}: Invalid Pokemon combat metadata type={} category={} power={}.",
				mtype:name(),
				incomingLua.pokemonType,
				incomingLua.pokemonCategory,
				incomingLua.pokemonPower
			)
			return nil
		end
	end
	if incomingLua.castVisual and spell.setCastVisual then
		local visual = incomingLua.castVisual
		spell:setCastVisual(
			visual.north or 0,
			visual.east or 0,
			visual.south or 0,
