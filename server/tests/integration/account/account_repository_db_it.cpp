...
namespace it_account_repo_db {
	...
	inline std::string getSessionToken(const TestIds &ids) {
		return transformToSHA256(ids.name);
	}
	...
	inline time_t createAccount(Database &db, const TestIds &ids, const std::string &password = "") {
		...
		const auto sessionExpires = getTimeNow() + 3600;
		if (!db.executeQuery(fmt::format("INSERT INTO `account_sessions` (`id`, `account_id`, `expires`) "
		                                 "VALUES ('{}', {}, {})",
		                                 transformToSHA256(getSessionToken(ids)), ids.id, sessionExpires))) {
			for (const auto &log : AccountRepositoryDBTest::logger->logs) {
				if (log.level == "error") {
	...
	inline void assertSessionLoad(const TestIds &ids, time_t expectedLastDay) {
		AccountRepositoryDB accRepo {};
		auto acc = std::make_unique<AccountInfo>();
		ASSERT_TRUE(accRepo.loadBySession(getSessionToken(ids), acc));
		assertAccountLoad(*acc, ids, expectedLastDay);
		EXPECT_GT(acc->sessionExpires, getTimeNow());
	}
	...
	TEST_F(AccountRepositoryDBTest, LoadBySession) {
		...
		databaseTest(db, [&db] {
			auto ids = getTestIds();
			auto expectedLastDay = createAccount(db, ids);
			assertSessionLoad(ids, expectedLastDay);
		})();
	}

	TEST_F(AccountRepositoryDBTest, LoadBySessionRejectsLegacySHA1SessionId) {
		auto &db = g_database();
		databaseTest(db, [&db] {
			AccountRepositoryDB accRepo {};
			auto ids = getTestIds();
			...
			ASSERT_TRUE(db.executeQuery(fmt::format(
				...
				db.escapeString(transformToSHA1(getSessionToken(ids))),
				ids.id
			)));
			auto acc = std::make_unique<AccountInfo>();
			EXPECT_FALSE(accRepo.loadBySession(getSessionToken(ids), acc));
		})();
	}

	TEST_F(AccountRepositoryDBTest, LoadBySessionRejectsExpiredSession) {
		auto &db = g_database();
		databaseTest(db, [&db] {
			AccountRepositoryDB accRepo {};
			auto ids = getTestIds();
			ASSERT_NO_FATAL_FAILURE(createAccount(db, ids));
			ASSERT_TRUE(db.executeQuery(fmt::format(
				"UPDATE `account_sessions` SET `expires` = {} WHERE `account_id` = {}",
				getTimeNow(),
				ids.id
			)));
			auto acc = std::make_unique<AccountInfo>();
	...
	TEST_F(AccountRepositoryDBTest, LoadBySessionRejectsMalformedRawToken) {
		auto &db = g_database();
		databaseTest(db, [&db] {
			AccountRepositoryDB accRepo {};
			auto ids = getTestIds();
			ASSERT_NO_FATAL_FAILURE(createAccount(db, ids));
			auto acc = std::make_unique<AccountInfo>();
			EXPECT_FALSE(accRepo.loadBySession(ids.name, acc));
		})();
	}
