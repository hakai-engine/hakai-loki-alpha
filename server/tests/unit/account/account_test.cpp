...
#include "account/account_session.hpp"
...
TEST(AccountSessionTokenTest, AcceptsExactly64HexCharacters) {
	EXPECT_TRUE(AccountSession::isValidRawToken(std::string(64, '0')));
	EXPECT_TRUE(AccountSession::isValidRawToken(std::string(64, 'a')));
	EXPECT_TRUE(AccountSession::isValidRawToken(std::string(64, 'F')));
}

TEST(AccountSessionTokenTest, RejectsIncorrectLengthOrNonHexCharacters) {
	EXPECT_FALSE(AccountSession::isValidRawToken(std::string(63, 'a')));
	EXPECT_FALSE(AccountSession::isValidRawToken(std::string(65, 'a')));

	auto nonHexToken = std::string(64, 'a');
	nonHexToken[32] = 'g';
	EXPECT_FALSE(AccountSession::isValidRawToken(nonHexToken));

	auto tokenWithWhitespace = std::string(64, 'a');
	tokenWithWhitespace[32] = '\n';
	EXPECT_FALSE(AccountSession::isValidRawToken(tokenWithWhitespace));
}
...
TEST_F(AccountTest, FailedPasswordAuthenticationDoesNotLogCredentialHash) {
	Account acc { 1 };
	repository().addAccount(
		"canary@test.com",
		AccountInfo { 1, 1, 1, AccountType::ACCOUNT_TYPE_GOD, { { "Canary", 1 } } }
	);

	EXPECT_TRUE(eqEnum(acc.load(), AccountErrors_t::Ok));
	const std::string storedHash = "7c4a8d09ca3762af61e59520943dc26494f8941b";
	repository().password_ = storedHash;
	EXPECT_FALSE(acc.authenticate("incorrect-password"));

	ASSERT_FALSE(testLogger().logs.empty());
	const auto &message = testLogger().logs.back().message;
	EXPECT_EQ(std::string { "Password authentication failed for account[1]." }, message);
	EXPECT_EQ(std::string::npos, message.find(storedHash));
	EXPECT_EQ(std::string::npos, message.find("incorrect-password"));
}
...
TEST_F(AccountTest, AuthenticateUsingSessionsRejectsExpiredOrBoundaryExpiration) {
	Account expiredAccount { 1 };
	repository().addAccount(
		"expired-session",
		AccountInfo {
			1,
	...
	EXPECT_TRUE(eqEnum(expiredAccount.load(), AccountErrors_t::Ok));
	EXPECT_FALSE(expiredAccount.authenticate());

	Account boundaryAccount { 2 };
	repository().addAccount(
		"boundary-session",
		AccountInfo {
			2,
	...
	EXPECT_TRUE(eqEnum(boundaryAccount.load(), AccountErrors_t::Ok));
	EXPECT_FALSE(boundaryAccount.authenticate());
}

TEST_F(AccountTest, GameWorldAdmissionAllowsTrainerVocation) {
	repository().addAccount(
		"session-key",
		AccountInfo {
			1,
	...
	repository().setCharacterVocation(1, "Trainer", 11);

	Account account { "session-key" };
	ASSERT_TRUE(eqEnum(account.load(), AccountErrors_t::Ok));
	EXPECT_TRUE(account.canCharacterEnterGameWorld("Trainer"));
}

TEST_F(AccountTest, GameWorldAdmissionRejectsNonTrainerVocation) {
	repository().addAccount(
		"session-key",
		AccountInfo {
			1,
	...
	repository().setCharacterVocation(1, "Knight", 4);

	Account account { "session-key" };
	ASSERT_TRUE(eqEnum(account.load(), AccountErrors_t::Ok));
	EXPECT_FALSE(account.canCharacterEnterGameWorld("Knight"));
}

TEST_F(AccountTest, GameWorldAdmissionRejectsMissingCharacter) {
	repository().addAccount(
		"session-key",
		AccountInfo {
			1,
	...
	Account account { "session-key" };
	ASSERT_TRUE(eqEnum(account.load(), AccountErrors_t::Ok));
	EXPECT_FALSE(account.canCharacterEnterGameWorld("Other Trainer"));
}

TEST_F(AccountTest, GameWorldAdmissionRejectsDeletedTrainer) {
	repository().addAccount(
		"session-key",
		AccountInfo {
			1,
			...
			1,
			AccountType::ACCOUNT_TYPE_GOD,
			{ { "Deleted Trainer", getTimeNow() + 1000 } },
			false,
			getTimeNow() + 24 * 60 * 60 * 1000 }
	);
	repository().setCharacterVocation(1, "Deleted Trainer", 11);

	Account account { "session-key" };
	ASSERT_TRUE(eqEnum(account.load(), AccountErrors_t::Ok));
	EXPECT_FALSE(account.canCharacterEnterGameWorld("Deleted Trainer"));
}

TEST_F(AccountTest, GameWorldAdmissionRejectsBeforeAccountLoad) {
	Account account { "session-key" };
	EXPECT_FALSE(account.canCharacterEnterGameWorld("Trainer"));
}
