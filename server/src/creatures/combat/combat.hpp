...
struct CombatParams {
	...
	PokemonType_t pokemonType = PokemonType_t::NONE;
	PokemonMoveCategory_t pokemonCategory = PokemonMoveCategory_t::NONE;
	uint16_t pokemonPower = 0;
...
public:
	...
	void setPokemonMetadata(PokemonType_t type, PokemonMoveCategory_t category, uint16_t power);

	static double getPokemonTypeEffectiveness(PokemonType_t attackType, const std::array<PokemonType_t, 2> &defenderTypes);
	static int32_t calculatePokemonDamage(
