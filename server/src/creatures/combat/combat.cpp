...
namespace {
constexpr std::array<std::array<uint16_t, POKEMON_TYPE_COUNT + 1>, POKEMON_TYPE_COUNT + 1> pokemonTypeChart = {{
	{},
	{ 0, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 50, 0, 100, 100, 50, 100 }, // Normal
	{ 0, 100, 50, 50, 100, 200, 200, 100, 100, 100, 100, 100, 200, 50, 100, 50, 100, 200, 100 }, // Fire
	{ 0, 100, 200, 50, 100, 50, 100, 100, 100, 200, 100, 100, 100, 200, 100, 50, 100, 100, 100 }, // Water
	{ 0, 100, 100, 200, 50, 50, 100, 100, 100, 0, 200, 100, 100, 100, 100, 50, 100, 100, 100 }, // Electric
	{ 0, 100, 50, 200, 100, 50, 100, 100, 50, 200, 50, 100, 50, 200, 100, 50, 100, 50, 100 }, // Grass
	{ 0, 100, 50, 50, 100, 200, 50, 100, 100, 200, 200, 100, 100, 200, 100, 200, 100, 50, 100 }, // Ice
	{ 0, 200, 100, 100, 100, 100, 200, 100, 50, 100, 50, 50, 50, 200, 0, 100, 200, 200, 50 }, // Fighting
	{ 0, 100, 100, 100, 100, 200, 100, 100, 50, 50, 100, 100, 100, 50, 50, 100, 100, 0, 200 }, // Poison
	{ 0, 100, 200, 100, 200, 50, 100, 100, 200, 100, 0, 100, 50, 200, 100, 100, 100, 200, 100 }, // Ground
	{ 0, 100, 100, 100, 50, 200, 100, 200, 100, 100, 100, 100, 200, 50, 100, 100, 100, 50, 100 }, // Flying
	{ 0, 100, 100, 100, 100, 100, 100, 200, 200, 100, 100, 50, 100, 100, 100, 100, 0, 50, 100 }, // Psychic
	{ 0, 100, 50, 100, 100, 200, 100, 50, 50, 100, 50, 200, 100, 100, 50, 100, 200, 50, 50 }, // Bug
	{ 0, 100, 200, 100, 100, 100, 200, 50, 100, 50, 200, 100, 200, 100, 100, 100, 100, 50, 100 }, // Rock
	{ 0, 0, 100, 100, 100, 100, 100, 100, 100, 100, 100, 200, 100, 100, 200, 100, 50, 100, 100 }, // Ghost
	{ 0, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 200, 100, 50, 0 }, // Dragon
	{ 0, 100, 100, 100, 100, 100, 100, 50, 100, 100, 100, 200, 200, 100, 200, 100, 50, 100, 50 }, // Dark
	{ 0, 100, 50, 50, 50, 100, 200, 100, 100, 100, 100, 100, 100, 200, 100, 100, 100, 50, 200 }, // Steel
	{ 0, 100, 50, 100, 100, 100, 100, 200, 50, 100, 100, 100, 100, 100, 100, 200, 200, 50, 100 }, // Fairy
}};

constexpr uint16_t POKEMON_BASE_CRITICAL_CHANCE = 416;
constexpr double POKEMON_CRITICAL_MULTIPLIER = 1.5;
constexpr double POKEMON_STAB_MULTIPLIER = 1.5;
}
...
double Combat::getPokemonTypeEffectiveness(PokemonType_t attackType, const std::array<PokemonType_t, 2> &defenderTypes) {
	const auto attackIndex = static_cast<uint8_t>(attackType);
	if (attackIndex == 0 || attackIndex > POKEMON_TYPE_COUNT) {
		return 1.0;
	}

	double multiplier = 1.0;
	for (const auto defenderType : defenderTypes) {
		const auto defenderIndex = static_cast<uint8_t>(defenderType);
		if (defenderIndex == 0) {
			continue;
		}
		if (defenderIndex > POKEMON_TYPE_COUNT) {
			return 1.0;
		}
		multiplier *= static_cast<double>(pokemonTypeChart[attackIndex][defenderIndex]) / 100.0;
	}
	return multiplier;
}

int32_t Combat::calculatePokemonDamage(
	const PokemonCombatProfile &attacker,
	const PokemonCombatProfile &target,
	PokemonType_t moveType,
	PokemonMoveCategory_t category,
	uint16_t power,
	uint8_t variancePercent,
	bool critical
) {
	if (category == PokemonMoveCategory_t::STATUS || power == 0) {
		return 0;
	}

	const uint32_t offensiveStat = category == PokemonMoveCategory_t::PHYSICAL
		? attacker.attack
		: attacker.specialAttack;
	const uint32_t defensiveStat = category == PokemonMoveCategory_t::PHYSICAL
		? target.defense
		: target.specialDefense;
	const double effectiveness = getPokemonTypeEffectiveness(moveType, target.types);
	if (effectiveness == 0.0) {
		return 0;
	}

	const uint32_t effectiveLevel = std::max<uint32_t>(1, attacker.level);
	const double levelFactor = std::floor((2.0 * effectiveLevel) / 5.0) + 2.0;
	const double baseDamage = std::floor(
		std::floor(levelFactor * power * std::max<uint32_t>(1, offensiveStat) / std::max<uint32_t>(1, defensiveStat)) / 50.0
	) + 2.0;
	const double stab = attacker.hasType(moveType) ? POKEMON_STAB_MULTIPLIER : 1.0;
	const double criticalMultiplier = critical ? POKEMON_CRITICAL_MULTIPLIER : 1.0;
	const double variance = std::clamp<uint8_t>(variancePercent, 85, 100) / 100.0;
	const double gender = std::max(0.0, attacker.genderDamageMultiplier);
	return std::max<int32_t>(1, static_cast<int32_t>(std::floor(
		baseDamage * stab * effectiveness * criticalMultiplier * variance * gender
	)));
}
...
CombatDamage Combat::getCombatDamage(const std::shared_ptr<Creature> &creature, const std::shared_ptr<Creature> &target) const {
	...
	damage.pokemonType = params.pokemonType;
	damage.pokemonCategory = params.pokemonCategory;
	damage.pokemonPower = params.pokemonPower;
...
ReturnValue Combat::canTargetCreature(const std::shared_ptr<Player> &player, const std::shared_ptr<Creature> &target) {
	...
	if (!player->getVocation()->canCombat()) {
...
ReturnValue Combat::canDoCombat(const std::shared_ptr<Creature> &caster, const std::shared_ptr<Tile> &tile, bool aggressive) {
	...
	if (casterPlayer && !casterPlayer->getVocation()->canCombat()) {
		return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
	}
...
ReturnValue Combat::canDoCombat(const std::shared_ptr<Creature> &attacker, const std::shared_ptr<Creature> &target, bool aggressive) {
	...
	const auto &directAttackerPlayer = attacker ? attacker->getPlayer() : nullptr;
	if (directAttackerPlayer && !directAttackerPlayer->getVocation()->canCombat()) {
...
void Combat::setPokemonMetadata(PokemonType_t type, PokemonMoveCategory_t category, uint16_t power) {
	params.pokemonType = type;
	params.pokemonCategory = category;
	params.pokemonPower = power;
}
...
void Combat::CombatHealthFunc(const std::shared_ptr<Creature> &caster, const std::shared_ptr<Creature> &target, const CombatParams &params, CombatDamage* data) {
	...
	const bool pokemonDamage = damage.isPokemonDamage();

	if (pokemonDamage) {
		// AreaCombat already excludes its aggressive caster. Keep the same
		// invariant at the damage boundary so a malformed direct cast can never
		// turn an ordinary Pokemon move into implicit recoil.
		if (!caster || !target || caster == target) {
			return;
		}
		if (!attackerMonster || !attackerMonster->getPokemonCombatProfile().valid) {
			g_logger().error(
				"[{}] Pokemon damage from creature {} has no runtime combat profile",
				__FUNCTION__,
				caster->getName()
			);
			return;
		...
		PokemonCombatProfile targetProfile;
		if (targetMonster && targetMonster->getPokemonCombatProfile().valid) {
			targetProfile = targetMonster->getPokemonCombatProfile();
		} else {
			// Non-Pokemon development targets and trainers are neutral. They do
			// not gain a fictional Pokemon type, but still provide a bounded
			// defensive denominator for deterministic QA.
			targetProfile.valid = true;
			targetProfile.defense = std::max<int32_t>(1, target->getDefense());
			targetProfile.specialDefense = targetProfile.defense;
		}

		const bool critical = uniform_random(1, 10000) <= POKEMON_BASE_CRITICAL_CHANCE;
		const uint8_t variance = static_cast<uint8_t>(uniform_random(85, 100));
		const int32_t resolvedDamage = calculatePokemonDamage(
			attackerMonster->getPokemonCombatProfile(),
			targetProfile,
			damage.pokemonType,
			damage.pokemonCategory,
			damage.pokemonPower,
			variance,
			critical
		);
		damage.primary.type = COMBAT_NEUTRALDAMAGE;
		damage.primary.value = -resolvedDamage;
		damage.secondary.type = COMBAT_NONE;
		damage.secondary.value = 0;
		damage.critical = critical && resolvedDamage > 0;
	}

	if (!pokemonDamage && attackerPlayer) {
	...
	if (!pokemonDamage && attackerPlayer && targetMonster) {
	...
	if (!pokemonDamage && attackerMonster && targetPlayer) {
	...
	if (g_game().combatChangeHealth(caster, target, damage)) {
		...
		if (pokemonDamage || !targetMonster || !attackerPlayer) {
			return;
		}
