...
enum BestiaryType_t : uint8_t {
	...
	BESTY_RACE_LAST = BESTY_RACE_KANTO,
};
...
enum class PokemonType_t : uint8_t {
	NONE = 0,
	NORMAL = 1,
	FIRE = 2,
	WATER = 3,
...
inline constexpr uint8_t POKEMON_TYPE_COUNT = 18;

enum class PokemonMoveCategory_t : uint8_t {
	NONE = 0,
	PHYSICAL = 1,
	SPECIAL = 2,
	STATUS = 3,
...
inline constexpr uint8_t POKEMON_MOVE_CATEGORY_COUNT = 3;

static_assert(static_cast<uint8_t>(PokemonType_t::NORMAL) == 1);
static_assert(static_cast<uint8_t>(PokemonType_t::FAIRY) == POKEMON_TYPE_COUNT);
static_assert(static_cast<uint8_t>(PokemonMoveCategory_t::PHYSICAL) == 1);
static_assert(static_cast<uint8_t>(PokemonMoveCategory_t::STATUS) == POKEMON_MOVE_CATEGORY_COUNT);

struct PokemonCombatProfile {
	bool valid = false;
	uint16_t level = 1;
	std::array<PokemonType_t, 2> types { PokemonType_t::NONE, PokemonType_t::NONE };
	uint32_t attack = 1;
	uint32_t defense = 1;
	uint32_t specialAttack = 1;
	uint32_t specialDefense = 1;
	uint32_t speed = 1;
	double genderDamageMultiplier = 1.0;

	[[nodiscard]] bool hasType(PokemonType_t candidate) const {
		return candidate != PokemonType_t::NONE
			&& (types[0] == candidate || types[1] == candidate);
	}
};
...
struct CombatDamage {
	...
	bool isPokemonDamage() const {
		return origin == ORIGIN_POKEMON
			|| (pokemonType != PokemonType_t::NONE && pokemonCategory != PokemonMoveCategory_t::NONE);
	}

	bool isEmpty() const {
		return primary.type == COMBAT_NONE && primary.value == 0 && secondary.type == COMBAT_NONE && secondary.value == 0 && origin == ORIGIN_NONE && pokemonType == PokemonType_t::NONE && pokemonCategory == PokemonMoveCategory_t::NONE && pokemonPower == 0 && critical == false && affected == 1 && extension == false && exString.empty() && fatal == false && criticalDamage == 0 && criticalChance == 0 && damageMultiplier == 0 && damageReductionMultiplier == 0 && healingMultiplier == 0 && manaLeech == 0 && manaLeechChance == 0 && lifeLeech == 0 && lifeLeechChance == 0 && healingLink == 0 && instantSpellName.empty() && runeSpellName.empty();
	}
};
