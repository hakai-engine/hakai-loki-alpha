...
public:
	...
	void setPokemonCombatProfile(const PokemonCombatProfile &profile);
	[[nodiscard]] const PokemonCombatProfile &getPokemonCombatProfile() const;
	void setPokemonPlayerControlled(bool value);
	[[nodiscard]] bool isPokemonPlayerControlled() const;
	bool castPokemonMove(const std::string &moveId, const std::shared_ptr<Creature> &target);
...
private:
	...
	PokemonCombatProfile pokemonCombatProfile;
	bool pokemonPlayerControlled = false;
