...
void Monster::setPokemonCombatProfile(const PokemonCombatProfile &profile) {
	pokemonCombatProfile = profile;
}

const PokemonCombatProfile &Monster::getPokemonCombatProfile() const {
	return pokemonCombatProfile;
}

void Monster::setPokemonPlayerControlled(bool value) {
	pokemonPlayerControlled = value;
}

bool Monster::isPokemonPlayerControlled() const {
	return pokemonPlayerControlled;
}

bool Monster::castPokemonMove(const std::string &moveId, const std::shared_ptr<Creature> &target) {
	if (!pokemonPlayerControlled || isRemoved() || isDead() || moveId.empty()) {
		return false;
	}

	const auto spellIt = std::ranges::find_if(attackSpells, [&moveId](const spellBlock_t &spellBlock) {
		return spellBlock.pokemonMoveId == moveId;
	});
	if (spellIt == attackSpells.end() || spellIt->spell == nullptr) {
		return false;
	}

	const auto &spellBlock = *spellIt;
	const auto spellTarget = spellBlock.needTarget ? target : getMonster();
	if (!spellTarget || spellTarget->isLifeless()) {
		return false;
	}
	if (spellBlock.needTarget) {
		if (!target || !target->isAttackable() || !g_game().isSightClear(getPosition(), target->getPosition(), true)) {
			return false;
		}
		if (spellBlock.range != 0 && std::max<uint32_t>(Position::getDistanceX(getPosition(), target->getPosition()), Position::getDistanceY(getPosition(), target->getPosition())) > spellBlock.range) {
			return false;
		}
	...
	if (!spellBlock.spell->castSpell(getMonster(), spellTarget)) {
		return false;
	}

	uint16_t castVisual = 0;
	switch (getDirection()) {
		case DIRECTION_NORTH: castVisual = spellBlock.castVisualNorth; break;
		case DIRECTION_EAST: castVisual = spellBlock.castVisualEast; break;
		case DIRECTION_SOUTH: castVisual = spellBlock.castVisualSouth; break;
		case DIRECTION_WEST: castVisual = spellBlock.castVisualWest; break;
		default: break;
	}
	if (castVisual != 0) {
		Position visualPosition = getPosition();
		if (spellBlock.castVisualWidth > 0 && spellBlock.castVisualLength > 0) {
			const int32_t halfWidth = spellBlock.castVisualWidth / 2;
			switch (getDirection()) {
				case DIRECTION_NORTH: visualPosition.x += halfWidth; visualPosition.y -= 1; break;
				case DIRECTION_EAST: visualPosition.x += spellBlock.castVisualLength; visualPosition.y += halfWidth; break;
				case DIRECTION_SOUTH: visualPosition.x += halfWidth; visualPosition.y += spellBlock.castVisualLength; break;
				case DIRECTION_WEST: visualPosition.x -= 1; visualPosition.y += halfWidth; break;
				default: break;
			}
		}
		g_game().addMagicEffect(visualPosition, castVisual);
	}
	if (!spellBlock.castText.empty()) {
		g_game().internalCreatureSay(getMonster(), TALKTYPE_MONSTER_SAY, spellBlock.castText, false);
	}
	return true;
}
	...
	for (size_t spellIndex = 0; spellIndex < attackSpells.size(); ++spellIndex) {
		...
		if (canUseSpell(getPosition(), target->getPosition(), spellBlock, request.interval, inRange, resetTicks) && geometrySuggested) {
			if (spellBlock.chance >= static_cast<uint32_t>(uniform_random(1, 100))) {
				...
				if (spellBlock.spell->castSpell(getMonster(), target)) {
					uint16_t castVisual = 0;
					switch (getDirection()) {
						case DIRECTION_NORTH:
							castVisual = spellBlock.castVisualNorth;
							break;
						case DIRECTION_EAST:
							castVisual = spellBlock.castVisualEast;
							break;
						case DIRECTION_SOUTH:
							castVisual = spellBlock.castVisualSouth;
							break;
						case DIRECTION_WEST:
							castVisual = spellBlock.castVisualWest;
							break;
						default:
							break;
					}
					...
					if (castVisual != 0) {
						...
						if (spellBlock.castVisualWidth > 0 && spellBlock.castVisualLength > 0) {
							...
							switch (getDirection()) {
								case DIRECTION_NORTH:
									visualPosition.x += halfWidth;
									visualPosition.y -= 1;
									break;
								case DIRECTION_EAST:
									visualPosition.x += spellBlock.castVisualLength;
									visualPosition.y += halfWidth;
									break;
								case DIRECTION_SOUTH:
									visualPosition.x += halfWidth;
									visualPosition.y += spellBlock.castVisualLength;
									break;
								case DIRECTION_WEST:
									visualPosition.x -= 1;
									visualPosition.y += halfWidth;
