...
struct spellBlock_t {
	...
	// Stable domain ID used by controlled Pokemon casts. It is separate from
	// the technical spell name (usually "combat").
	std::string pokemonMoveId;
	std::string castText;
	uint16_t castVisualNorth = 0;
	uint16_t castVisualEast = 0;
	uint16_t castVisualSouth = 0;
	uint16_t castVisualWest = 0;
	uint8_t castVisualWidth = 0;
	uint8_t castVisualLength = 0;
	uint32_t chance = 100;
	uint32_t speed = 2000;
	uint32_t range = 0;
