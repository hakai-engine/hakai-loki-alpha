...
bool Monsters::deserializeSpell(const std::shared_ptr<MonsterSpell> &spell, spellBlock_t &sb, const std::string &description) const {
	...
	sb.pokemonMoveId = spell->pokemonMoveId.substr(0, 64);
	sb.needTarget = spell->needTarget;
	...
	sb.castText = spell->castText.substr(0, 64);
	sb.castVisualNorth = spell->castVisualNorth;
	sb.castVisualEast = spell->castVisualEast;
	sb.castVisualSouth = spell->castVisualSouth;
	sb.castVisualWest = spell->castVisualWest;
	sb.castVisualWidth = spell->castVisualWidth;
	sb.castVisualLength = spell->castVisualLength;
	...
	if (const std::string spellName = asLowerCaseString(spell->name);
		...
		const bool hasPokemonType = spell->pokemonType != PokemonType_t::NONE;
		const bool hasPokemonCategory = spell->pokemonCategory != PokemonMoveCategory_t::NONE;
		const bool hasPokemonPower = spell->pokemonPower > 0;
		if (hasPokemonType != hasPokemonCategory
		    || (hasPokemonCategory && spell->pokemonCategory != PokemonMoveCategory_t::STATUS && !hasPokemonPower)) {
			g_logger().error(
				"[Monsters::deserializeSpell] - Incomplete Pokemon combat metadata (type/category/power) for: {}",
				description
			);
			return false;
		}
		...
		if (hasPokemonType) {
			// CombatType_t is only the neutral HP transport for Pokemon hits.
			// PokemonType_t remains the sole gameplay type and ORIGIN_POKEMON
			// explicitly bypasses Tibia armor, elements and combat modifiers.
			combatPtr->setParam(COMBAT_PARAM_TYPE, COMBAT_NEUTRALDAMAGE);
			combatPtr->setOrigin(ORIGIN_POKEMON);
			combatPtr->setPokemonMetadata(spell->pokemonType, spell->pokemonCategory, spell->pokemonPower);
		} else {
