...
void Creature::onGainExperience(uint64_t gainExp, const std::shared_ptr<Creature> &target) {
	...
	// Player-controlled Pokemon use the Hakai persistent progression pipeline.
	// Do not leak Canary's native summon XP into the Trainer or show its
	// half-base-XP floating number; Lua publishes the authoritative instance XP.
	if (m && m->isPokemonPlayerControlled()) {
		return;
	}
	if (!m->isFamiliar()) {
		gainExp /= 2;
