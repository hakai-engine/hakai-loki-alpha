...
bool Game::combatBlockHit(CombatDamage &damage, const std::shared_ptr<Creature> &attacker, const std::shared_ptr<Creature> &target, bool checkDefense, bool checkArmor, bool field, bool condition /* = false */) {
	...
	// Pokemon hits are resolved by the Pokemon domain. CombatType_t is only a
	// neutral HP transport here; no Tibia dodge, armor, elements, buffs,
	// absorption, reflection, mitigation or defensive charms may reinterpret it.
	if (damage.isPokemonDamage()) {
		return false;
	}
...
int32_t Game::applyHealthChange(const CombatDamage &damage, const std::shared_ptr<Creature> &target) const {
	...
	if (!damage.isPokemonDamage()) {
		const std::shared_ptr<Player> targetPlayer = target->getPlayer();
		if (!targetPlayer) {
			return targetHealth;
		}
...
bool Game::combatChangeHealth(const std::shared_ptr<Creature> &attacker, const std::shared_ptr<Creature> &target, CombatDamage &damage, bool isEvent /*= false*/) {
	...
	if (damage.primary.value > 0) {
		...
		if (!pokemonDamage && damage.origin != ORIGIN_NONE && attacker && damage.primary.type != COMBAT_HEALING) {
			damage.primary.value *= attacker->getBuff(BUFF_DAMAGEDEALT) / 100.;
			damage.secondary.value *= attacker->getBuff(BUFF_DAMAGEDEALT) / 100.;
		}
		if (!pokemonDamage && damage.origin != ORIGIN_NONE && target && damage.primary.type != COMBAT_HEALING) {
			damage.primary.value *= target->getBuff(BUFF_DAMAGERECEIVED) / 100.;
			damage.secondary.value *= target->getBuff(BUFF_DAMAGERECEIVED) / 100.;
		}
		...
		if (!pokemonDamage && targetPlayer && attackerMonster) {
			handleHazardSystemAttack(damage, targetPlayer, attackerMonster, false);
		} else if (!pokemonDamage && attackerPlayer && targetMonster) {
		...
		if (!pokemonDamage && !damage.extension && attackerMonster && targetPlayer) {
			// Charm rune (target as player)
		...
		if (!pokemonDamage && target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
		...
		if (!pokemonDamage && attackerPlayer && targetPlayer) {
