...
bool AccountRepositoryDB::loadBySession(const std::string &sessionKey, std::unique_ptr<AccountInfo> &acc) {
	if (!AccountSession::isValidRawToken(sessionKey)) {
		return false;
	}
...
bool AccountRepositoryDB::getCharacterVocationByAccountIdAndName(const uint32_t &id, const std::string &name, uint16_t &vocationId) {
	const auto result = g_database().storeQuery(fmt::format(
		"SELECT `vocation` FROM `players` WHERE `account_id` = {} AND `name` = {} AND `deletion` = 0 LIMIT 1",
		id,
	...
	vocationId = result->getNumber<uint16_t>("vocation");
	return true;
}
