...
bool Account::canCharacterEnterGameWorld(const std::string &characterName) const {
	if (!m_accLoaded) {
		return false;
	}

	// World admission is an account-ownership check. Vocation is a gameplay
	// concern; requiring Trainer here blocks legitimate administrator and
	// migration characters before they can enter the world.
	uint16_t vocationId = 0;
	return g_accountRepository().getCharacterVocationByAccountIdAndName(m_account->id, characterName, vocationId);
}
...
bool Account::authenticateSession() {
	...
	if (m_account->sessionExpires <= currentTime) {
		g_logger().error("Session expired for account[{}] expired at [{}] current time [{}]!", m_account->id, m_account->sessionExpires, currentTime);
		return false;
	}
	return true;
}
...
bool Account::authenticatePassword(const std::string &password) {
	...
	g_logger().error("Password authentication failed for account[{}].", m_account->id);
	return false;
}
