...
bool IOLoginData::gameWorldAuthentication(const std::string &accountDescriptor, const std::string &password, std::string &characterName, uint32_t &accountId, bool oldProtocol, const uint32_t ip) {
	...
	if (AccountErrors_t::Ok != account.load()) {
		// accountDescriptor is a raw session key when authType=session.
		// Never include it in logs, even when loading the account fails.
		g_logger().error("Couldn't load account for world authentication from IP [{}].", convertIPToString(ip));
		return false;
	}
	...
	if (!account.canCharacterEnterGameWorld(characterName)) {
		g_logger().warn(
			"World admission denied for account ID [{}], character [{}], IP [{}].",
			account.getID(),
			characterName,
	...
	if (AccountErrors_t::Ok != account.load()) {
		g_logger().error("Failed to reload account ID [{}] during world authentication.", account.getID());
		return false;
	}

	auto [players, result] = account.getAccountPlayers();
	if (AccountErrors_t::Ok != result) {
		g_logger().error("Failed to load players for account ID [{}].", account.getID());
		return false;
	}

	const auto playerIt = players.find(characterName);
	if (playerIt == players.end() || playerIt->second != 0) {
		g_logger().error("Account ID [{}] player [{}] not found or deleted.", account.getID(), characterName);
		return false;
	}
