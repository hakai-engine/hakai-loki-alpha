...
void Signals::dispatchSignalHandler(int signal) {
	...
	switch (signal) {
		...
		case SIGBREAK: // Shuts the server down
			...
			// Windows terminates the process when this console handler returns.
			// Keep it alive while the normal lifecycle performs the complete
			// shutdown in CanaryServer::shutdown(). The process exits when the
			// main thread returns from serviceManager.run().
			for (;;) {
