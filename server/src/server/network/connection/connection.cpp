...
void Connection::onWriteOperation(const std::error_code &error) {
	...
	if (error) {
		// A client can close its socket immediately after logout while the final
		// server message is still queued. Treat expected disconnect conditions
		// like the read path does instead of reporting a server failure.
		if (error != asio::error::operation_aborted && error != asio::error::eof && error != asio::error::connection_reset) {
