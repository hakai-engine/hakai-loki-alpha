...
namespace {
	...
	[[nodiscard]] bool isSha256Identifier(const std::string_view value) {
		if (value.size() != 64) {
			return false;
		}
	...
	[[nodiscard]] bool equalSha256Identifiers(const std::string_view left, const std::string_view right) {
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage &msg) {
	...
	if (gameLoginLayout->hasAssetHashString) {
		...
		trimString(assetHashIdentifier);
		if (!isSha256Identifier(assetHashIdentifier)) {
			disconnectClient("Invalid client asset identifier.");
			return;
		}

		const auto &expectedAssetIdentifier = g_configManager().getString(CLIENT_ASSET_IDENTIFIER);
		if (!expectedAssetIdentifier.empty() && !equalSha256Identifiers(assetHashIdentifier, expectedAssetIdentifier)) {
			disconnectClient("This client asset catalog is not supported.");
			return;
		}
		g_logger().trace("Client asset hash identifier accepted.");
	}
