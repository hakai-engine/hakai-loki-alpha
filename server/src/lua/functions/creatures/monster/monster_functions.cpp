...
void MonsterFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "Monster", "setPokemonCombatProfile", MonsterFunctions::luaMonsterSetPokemonCombatProfile);
	Lua::registerMethod(L, "Monster", "setPokemonPlayerControlled", MonsterFunctions::luaMonsterSetPokemonPlayerControlled);
	Lua::registerMethod(L, "Monster", "castPokemonMove", MonsterFunctions::luaMonsterCastPokemonMove);
...
int MonsterFunctions::luaMonsterSetPokemonCombatProfile(lua_State* L) {
	// monster:setPokemonCombatProfile(level, type1, type2, attack, defense,
	//     specialAttack, specialDefense, speed, genderDamageMultiplier)
	const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
	if (!monster) {
		lua_pushnil(L);
		return 1;
	...
	const auto type1 = Lua::getNumber<PokemonType_t>(L, 3);
	const auto type2 = Lua::getNumber<PokemonType_t>(L, 4, PokemonType_t::NONE);
	const auto isValidType = [](PokemonType_t type, bool allowNone) {
		return (allowNone && type == PokemonType_t::NONE)
			|| (type >= PokemonType_t::NORMAL && type <= PokemonType_t::FAIRY);
	};
	if (!isValidType(type1, false) || !isValidType(type2, true)) {
		lua_pushnil(L);
		return 1;
	}
	...
	profile.valid = true;
	profile.level = std::max<uint16_t>(1, Lua::getNumber<uint16_t>(L, 2, 1));
	profile.types = { type1, type2 };
	profile.attack = std::max<uint32_t>(1, Lua::getNumber<uint32_t>(L, 5, 1));
	// Development targets may intentionally expose zero defenses. The damage
	// formula applies a safe denominator of 1, so zero never divides by zero.
	profile.defense = Lua::getNumber<uint32_t>(L, 6, 1);
	profile.specialAttack = std::max<uint32_t>(1, Lua::getNumber<uint32_t>(L, 7, 1));
	profile.specialDefense = Lua::getNumber<uint32_t>(L, 8, 1);
	profile.speed = std::max<uint32_t>(1, Lua::getNumber<uint32_t>(L, 9, 1));
	profile.genderDamageMultiplier = std::clamp(Lua::getNumber<double>(L, 10, 1.0), 0.0, 10.0);
	monster->setPokemonCombatProfile(profile);
	Lua::pushBoolean(L, true);
	return 1;
}

int MonsterFunctions::luaMonsterSetPokemonPlayerControlled(lua_State* L) {
	// monster:setPokemonPlayerControlled(bool)
	const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
	if (!monster) {
		lua_pushnil(L);
		return 1;
	}
	monster->setPokemonPlayerControlled(Lua::getBoolean(L, 2, false));
	Lua::pushBoolean(L, true);
	return 1;
}

int MonsterFunctions::luaMonsterCastPokemonMove(lua_State* L) {
	// monster:castPokemonMove(moveId, target)
	const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
	if (!monster) {
		lua_pushnil(L);
		return 1;
	}
	const auto &target = Lua::getUserdataShared<Creature>(L, 3, "Creature");
	Lua::pushBoolean(L, monster->castPokemonMove(Lua::getString(L, 2), target));
	return 1;
}
