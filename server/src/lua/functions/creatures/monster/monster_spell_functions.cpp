...
void MonsterSpellFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "MonsterSpell", "setCastText", MonsterSpellFunctions::luaMonsterSpellSetCastText);
	Lua::registerMethod(L, "MonsterSpell", "setPokemonMoveId", MonsterSpellFunctions::luaMonsterSpellSetPokemonMoveId);
	Lua::registerMethod(L, "MonsterSpell", "setCastVisual", MonsterSpellFunctions::luaMonsterSpellSetCastVisual);
	...
	Lua::registerMethod(L, "MonsterSpell", "setPokemonCombat", MonsterSpellFunctions::luaMonsterSpellSetPokemonCombat);
...
int MonsterSpellFunctions::luaMonsterSpellSetCastText(lua_State* L) {
	// monsterSpell:setCastText(text)
	const auto &spell = Lua::getUserdataShared<MonsterSpell>(L, 1, "MonsterSpell");
	if (spell) {
		spell->castText = Lua::getString(L, 2);
		Lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int MonsterSpellFunctions::luaMonsterSpellSetPokemonMoveId(lua_State* L) {
	// monsterSpell:setPokemonMoveId(moveId)
	const auto &spell = Lua::getUserdataShared<MonsterSpell>(L, 1, "MonsterSpell");
	if (spell) {
		spell->pokemonMoveId = Lua::getString(L, 2);
		Lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int MonsterSpellFunctions::luaMonsterSpellSetCastVisual(lua_State* L) {
	// monsterSpell:setCastVisual(north, east, south, west, width, length)
	const auto &spell = Lua::getUserdataShared<MonsterSpell>(L, 1, "MonsterSpell");
	if (spell) {
		spell->castVisualNorth = Lua::getNumber<uint16_t>(L, 2);
		spell->castVisualEast = Lua::getNumber<uint16_t>(L, 3);
		spell->castVisualSouth = Lua::getNumber<uint16_t>(L, 4);
		spell->castVisualWest = Lua::getNumber<uint16_t>(L, 5);
		spell->castVisualWidth = Lua::getNumber<uint8_t>(L, 6, 0);
		spell->castVisualLength = Lua::getNumber<uint8_t>(L, 7, 0);
		Lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int MonsterSpellFunctions::luaMonsterSpellSetPokemonCombat(lua_State* L) {
	// monsterSpell:setPokemonCombat(pokemonType, pokemonMoveCategory, power)
	const auto &spell = Lua::getUserdataShared<MonsterSpell>(L, 1, "MonsterSpell");
	if (spell) {
		const auto pokemonType = Lua::getNumber<PokemonType_t>(L, 2);
		const auto pokemonCategory = Lua::getNumber<PokemonMoveCategory_t>(L, 3);
		const auto pokemonPower = Lua::getNumber<uint16_t>(L, 4, 0);
		if (pokemonType <= PokemonType_t::NONE || pokemonType > PokemonType_t::FAIRY
		    || pokemonCategory <= PokemonMoveCategory_t::NONE || pokemonCategory > PokemonMoveCategory_t::STATUS
		    || (pokemonCategory != PokemonMoveCategory_t::STATUS && pokemonPower == 0)) {
			lua_pushnil(L);
			return 1;
		}

		spell->pokemonType = pokemonType;
		spell->pokemonCategory = pokemonCategory;
		spell->pokemonPower = pokemonPower;
		Lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
