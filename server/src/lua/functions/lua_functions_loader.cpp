...
void Lua::pushFullCombatDamage(lua_State* L, const CombatDamage &damage) {
	...
	lua_pushnumber(L, static_cast<uint8_t>(damage.pokemonType));
	lua_setfield(L, -2, "pokemonType");

	lua_pushnumber(L, static_cast<uint8_t>(damage.pokemonCategory));
	lua_setfield(L, -2, "pokemonCategory");

	lua_pushboolean(L, damage.isPokemonDamage());
	lua_setfield(L, -2, "isPokemonDamage");
