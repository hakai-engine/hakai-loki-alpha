...
bool ConfigManager::load() {
	...
	if (!loaded.load(std::memory_order_relaxed)) {
		...
		const auto clientAssetIdentifier = loadStringConfig(L, CLIENT_ASSET_IDENTIFIER, "clientAssetIdentifier", "");
		bool validClientAssetIdentifier = clientAssetIdentifier.empty() || clientAssetIdentifier.size() == 64;
		for (const auto c : clientAssetIdentifier) {
			if (std::isxdigit(static_cast<unsigned char>(c)) == 0) {
				validClientAssetIdentifier = false;
				break;
			}
		}
		if (!validClientAssetIdentifier) {
			g_logger().error("[ConfigManager::load] - clientAssetIdentifier must be empty or a 64-character SHA-256.");
			lua_close(L);
			return false;
		}
	...
	loadBoolConfig(L, POKEMON_GENDER_ENABLED, "pokemonGenderEnabled", true);
	loadBoolConfig(L, POKEMON_IVS_ENABLED, "pokemonIvsEnabled", true);
	loadBoolConfig(L, POKEMON_NATURE_ENABLED, "pokemonNatureEnabled", true);
