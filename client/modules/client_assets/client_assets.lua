...
local function isSafeRelativePath(path)
  path = tostring(path or ''):gsub('\\', '/')
  if path == '' or startsWith(path, '/') or path:match('^%a:') or path:find('%z') then
    return false
  end
...
local function hasModernClientFilesAtPath(basePath, installPath)
  ...
  for _, entry in ipairs(catalog) do
    ...
    if type(entry.file) ~= 'string' or not isSafeRelativePath(entry.file) then
      return false
    end
...
local function hasInstalledModernClientFiles(version)
  return installFileExists(completeMarkerPath(version)) and hasModernClientFiles(version)
end
...
local function verifyDownloadedSha256(downloadPath, expectedSha256)
  if not expectedSha256 or expectedSha256 == '' then
    return false, string.format('Missing pinned SHA-256 for %s.', downloadPath)
  end
...
local function shouldInstallManifestEntry(entry, version, config)
  ...
  if not isSafeRelativePath(localfile) then
    return nil, 'Unsafe asset manifest path: ' .. tostring(localfile)
  end
...
local function installManifestEntries(config, descriptor, files, index, installed, total, callback)
  ...
  local selectedEntry, selectionError = shouldInstallManifestEntry(entry, descriptor.version, descriptor)
  if selectionError then
    return callback(false, selectionError)
  end
...
local function installFromManifest(config, descriptor, callback)
  ...
  if (descriptor.strictManifestSha256 or config.strictManifestSha256)
      and (not descriptor.manifestSha256 or descriptor.manifestSha256 == '') then
    return callback(false, 'Strict asset installation requires a manifest SHA-256 pinned in local configuration.')
  end
  ...
  activeDownload.operationId = httpGet(config, descriptor.manifestUrl, function(data, err)
    ...
    fetchManifestSha256(config, descriptor, function(expectedSha256)
      ...
      if (descriptor.strictManifestSha256 or config.strictManifestSha256) and not expectedSha256 then
        return callback(false, 'Missing pinned SHA-256 for the assets manifest.')
      end
...
local function installFromArchive(config, descriptor, callback)
  ...
  if not descriptor.archiveSha256 or descriptor.archiveSha256 == '' then
    return callback(false, 'Asset archive installation requires a pinned SHA-256.')
  end
...
function ensureClientVersion(version, callback)
  ...
  if isClientVersionInstalled(version) then
    return callback(true)
  end
  if config.enabled == false then
    return callback(false, 'Automatic asset installation is disabled. Install the verified Hakai 15.25 asset pack locally.')
  end
