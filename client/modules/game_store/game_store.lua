...
local function applyOfferPricePresentation(priceLabel, offer, affordable)
    local price = tonumber(offer.price) or 0
    local basePrice = tonumber(offer.basePrice) or 0
    local isSale = offer.state == GameStore.States.STATE_SALE and basePrice > price

    if isSale then
        local discount = math.floor((((basePrice - price) * 100) / basePrice) + 0.5)
        priceLabel:setText(string.format("%d  -%d%%", price, discount))
        priceLabel:setTooltip(string.format("%d%% OFF (original price: %d)", discount, basePrice))
        priceLabel:setColor(affordable and "#56d364" or "#d33c3c")
    else
        priceLabel:setText(price)
        priceLabel:setTooltip("")
        priceLabel:setColor(affordable and "white" or "#d33c3c")
    end
end

local function setImagenHttp(widget, url, isIcon)
    -- Canary sends Store paths with a leading slash. Normalize once so both
    -- the packaged resource path and an optional HTTP base URL are stable.
    url = url:gsub("^/+", "")
...
local function refreshStoreAffordability()
    ...
    if listProduct then
        for _, row in ipairs(listProduct:getChildren()) do
            local offers = row.product and (row.product.subOffers or { row.product }) or {}
            local stackOffers = row:getChildById('StackOffers')
            local widgets = stackOffers and stackOffers:getChildren() or {}
            for index, offerWidget in ipairs(widgets) do
                local offer = offers[index]
                local priceLabel = offerWidget:getChildById('lblPrice')
                if offer and priceLabel then
                    local isTransferable = offer.coinType == GameStore.CoinType.Transferable
                    local balance = isTransferable and (normalCoins + transferableCoins) or normalCoins
                    applyOfferPricePresentation(priceLabel, offer, balance >= offer.price)
                end
            end
        end
...
function onStoreInit(url, coinsPacketSize)
    -- An empty or malformed URL must select the packaged local assets.
    -- Lua treats "" as truthy, so assigning it here makes every Store icon
    -- enter HTTP.downloadImage and produces an endless retry/error stream.
...
function onParseStoreGetCoin(getTibiaCoins, getTransferableCoins)
    ...
    controllerShop:scheduleEvent(refreshStoreAffordability, 1, 'refreshStoreAffordability')
end
...
--[[         comboBox.onOptionChange = function(a, b, c, d)
    ...
    for _, product in ipairs(storeProducts.offers) do
        ...
        for _, subOffer in ipairs(subOffers) do
            ...
            local balance = isTransferable and (coinsBalance1 + coinsBalance2) or coinsBalance2
            applyOfferPricePresentation(priceLabel, subOffer, balance >= price)
...
function onParseStoreCreateHome(offer)
    ...
    for _, product in ipairs(offer.offers) do
        ...
        applyOfferPricePresentation(subOfferWidget.lblPrice, product, true)
...
function chooseOffert(self, focusedChild)
    ...
    for _, offer in ipairs(offers) do
        ...
        local isTransferable = offer.coinType == GameStore.CoinType.Transferable
        local currentBalance = isTransferable and (normalCoins + transferableCoins) or normalCoins
        ...
        applyOfferPricePresentation(priceLabel, offer, currentBalance >= offer.price)
        ...
        offerPanel:getChildById('btnBuy').onClick = function(widget)
            ...
            local function acceptFunc()
                ...
                local latestCurrentBalance = isTransferable and (latestNormal + latestTransferable) or latestNormal
            ...
            local coinType = isTransferable and "coins (regular + transferable)" or "regular coins"
