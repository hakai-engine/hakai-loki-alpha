local UI = nil
local KANTO_BESTIARY = dofile("kanto_bestiary")

local KANTO_HABITATS = {
    { key = "all", label = "Todos os habitats" },
    { key = "routes", label = "Rotas" },
    { key = "forests", label = "Florestas" },
    { key = "caves", label = "Cavernas" },
    { key = "mountains", label = "Montanhas" },
    { key = "waters", label = "Aguas" },
    { key = "power_plant", label = "Power Plant" },
    { key = "pokemon_tower", label = "Pokemon Tower" },
    { key = "safari_zone", label = "Safari Zone" },
    { key = "legendary", label = "Lendarios" },
}

local KANTO_TYPES = {
    "Todos os tipos", "normal", "fire", "water", "electric", "grass", "ice",
    "fighting", "poison", "ground", "flying", "psychic", "bug", "rock",
    "ghost", "dragon", "dark", "steel", "fairy",
}

local function getBestiaryRaceData(raceId)
    raceId = tonumber(raceId)
    local nativeData = g_things.getRaceData(raceId)
    if nativeData and nativeData.outfit and tonumber(nativeData.outfit.type) and nativeData.outfit.type > 0 then
        return nativeData
    end

    local metadata = KANTO_BESTIARY[raceId]
    if metadata then
        return {
            raceId = raceId,
            name = metadata.name,
            outfit = { type = metadata.lookType },
        }
    end
    return nativeData
end

local function tableContains(values, wanted)
...
local function selectedOptionText(combo)
    local option = combo and combo:getCurrentOption()
    return option and option.text or nil
end

local function setKantoFiltersVisible(visible)
    if not UI then return end
    UI.KantoFilterLabel:setVisible(visible)
    UI.KantoHabitatFilter:setVisible(visible)
    UI.KantoTypeFilter:setVisible(visible)
    UI.ListBase:setMarginTop(visible and 38 or 10)
end

local function currentHabitatKey()
    local selected = selectedOptionText(UI and UI.KantoHabitatFilter)
    for _, habitat in ipairs(KANTO_HABITATS) do
        if habitat.label == selected then
            return habitat.key
        end
    end
    return "all"
...
local function filterKantoCreatures()
    if not UI or not Cyclopedia.Bestiary.KantoCreatures then return end
    local habitat = currentHabitatKey()
    local pokemonType = selectedOptionText(UI.KantoTypeFilter) or "Todos os tipos"
    local filtered = {}
    for _, creature in ipairs(Cyclopedia.Bestiary.KantoCreatures) do
        local metadata = KANTO_BESTIARY[tonumber(creature.id)]
        if metadata then
            local habitatMatches = habitat == "all" or tableContains(metadata.habitats, habitat)
            local typeMatches = pokemonType == "Todos os tipos" or tableContains(metadata.types, pokemonType)
            if habitatMatches and typeMatches then
                table.insert(filtered, creature)
            end
        end
    end
    Cyclopedia.Bestiary.Page = 1
    Cyclopedia.loadBestiaryCreatures(filtered, true)
end

local function setupKantoFilters()
    UI.KantoHabitatFilter:clearOptions()
    for _, habitat in ipairs(KANTO_HABITATS) do
        UI.KantoHabitatFilter:addOption(habitat.label)
    end
    UI.KantoTypeFilter:clearOptions()
    for _, pokemonType in ipairs(KANTO_TYPES) do
        UI.KantoTypeFilter:addOption(pokemonType)
    end
    UI.KantoHabitatFilter.onOptionChange = filterKantoCreatures
    UI.KantoTypeFilter.onOptionChange = filterKantoCreatures
end
...
function Cyclopedia.loadBestiaryOverview(name, creatures, animusMasteryPoints)
    if name == "Kanto" then
        Cyclopedia.Bestiary.KantoCreatures = creatures
        setKantoFiltersVisible(true)
        filterKantoCreatures()
        return
    end
    setKantoFiltersVisible(false)
...
function Cyclopedia.loadBestiarySelectedCreature(data)
    ...
    local raceData = getBestiaryRaceData(data.id)
...
function Cyclopedia.CreateBestiaryCategoryItem(Data)
    ...
    function widget.ClassBase:onClick()
        ...
        Cyclopedia.ShowBestiaryCreatures(categoryName)
...
function Cyclopedia.loadBestiaryCreatures(data, isKantoFiltered)
    if not isKantoFiltered then
        setKantoFiltersVisible(false)
    end
...
function Cyclopedia.loadBestiaryCategories(data)
    ...
    for _, categoryData in ipairs(data) do
        if categoryData.bestClass == "Kanto" and categoryData.count > 0 then
            table.insert(visibleCategories, categoryData)
        end
    end
    Cyclopedia.Bestiary.TotalCategoriesPages = math.max(1, math.ceil(#visibleCategories / maxCategoriesPerPage))
    ...
    for i = 1, #visibleCategories do
        ...
        local category = {
            name = visibleCategories[i].bestClass,
            amount = visibleCategories[i].count,
            know = visibleCategories[i].unlockedCount,
            AnimusMasteryBonus = visibleCategories[i].AnimusMasteryBonus,
        }
...
function Cyclopedia.onParseCyclopediaTracker(trackerType, data)
    ...
    for _, entry in ipairs(data) do
        ...
        local raceData = getBestiaryRaceData(raceId)
...
function Cyclopedia.sortTrackerData(data, trackerType)
    ...
    if filters.sortByName then
        table.sort(sortedData, function(a, b)
            local nameA = getBestiaryRaceData(a[1]).name:lower()
            local nameB = getBestiaryRaceData(b[1]).name:lower()
