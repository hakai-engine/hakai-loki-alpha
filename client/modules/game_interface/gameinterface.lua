...
function onGameStart()
    ...
    updateSidePanelButtons()
    applyMobileMargins()
    g_modules.ensureModuleLoaded("game_pokemon_roster")
end

function onGameEnd()
    hide()
    addEvent(function()
        local rosterModule = g_modules.getModule("game_pokemon_roster")
        if rosterModule and rosterModule:isLoaded() then
            rosterModule:unload()
        end
    end)
end
    ...
    keyboardModifiers == KeyboardNoModifier then
        ...
        if player then
            ...
            if playerPos.z == npcPos.z then
                ...
                if dist <= 3 and npcName:lower() == 'nurse joy' and
                   modules.game_hakai_nurse_joy and
                   modules.game_hakai_nurse_joy.requestInteraction(creatureThing:getId()) then
                    return true
                end
            end
        end
...
function setupViewMode(mode)
    ...
    if currentViewMode == 2 then
        ...
        gameLeftPanel:setImageColor('#ffffffb8')
        gameRightPanel:setImageColor('#ffffffb8')
        gameRightExtraPanel:setImageColor('#ffffffb8')
        gameLeftExtraPanel:setImageColor('#ffffffb8')
...
function applyExtendedViewLayout(extendedView)
    ...
    if extendedView then
        ...
        gameMainRightPanel:setImageColor('#ffffffb8')
