...
local activeHttpLogin
local nextHttpRequestId = 0

local function getUniqueServerConfig()
    if type(Servers_init) ~= 'table' then
        return nil, nil
    end
    ...
    if count ~= 1 or type(uniqueHost) ~= 'string' or type(uniqueValues) ~= 'table' then
        return nil, nil
    end
    return uniqueHost, uniqueValues
end

-- private functions
local function cancelHttpLogin()
    if activeHttpLogin then
        activeHttpLogin:cancel()
        activeHttpLogin = nil
    end
    G.requestId = 0
end
...
local function updateLabelText()
    if enterGame:getChildById('clientComboBox') and tonumber(enterGame:getChildById('clientComboBox'):getText()) > 1080 then
        enterGame:setText("POKEMON LOGIN")
...
function EnterGame.init()
    ...
    local uniqueHost, uniqueValues = getUniqueServerConfig()

    if uniqueHost then
        host = uniqueHost
        port = tonumber(uniqueValues.port)
        clientVersion = tonumber(uniqueValues.protocol)
        httpLogin = uniqueValues.httpLogin == true
    end
    ...
    local serverData = type(host) == 'string' and servers[host] or {}
    ...
    if uniqueHost then
        enterGame.disableToken = not uniqueValues.useAuthenticator
        EnterGame.setUniqueServer(uniqueHost, uniqueValues.port, uniqueValues.protocol)
        EnterGame.setHttpLogin(uniqueValues.httpLogin == true)
    elseif Servers_init and next(Servers_init) ~= nil then
...
function EnterGame.tryHttpLogin(clientVersion, httpLogin)
    ...
    nextHttpRequestId = nextHttpRequestId + 1
    if nextHttpRequestId > 2147483647 then
        nextHttpRequestId = 1
    end
    G.requestId = nextHttpRequestId

    activeHttpLogin = LoginHttp.create()
    activeHttpLogin:httpLogin(host, path, G.port, G.account, G.password, G.requestId, httpLogin, G.authenticatorToken)
end
...
function EnterGame.loginSuccess(requestId, jsonSession, jsonWorlds, jsonCharacters)
    ...
    local worldsOk, decodedWorlds = pcall(json.decode, jsonWorlds)
    local charactersOk, decodedCharacters = pcall(json.decode, jsonCharacters)
    local sessionOk, session = pcall(json.decode, jsonSession)
    if not worldsOk or type(decodedWorlds) ~= 'table'
        or not charactersOk or type(decodedCharacters) ~= 'table'
        or not sessionOk or type(session) ~= 'table'
        or type(session.sessionkey) ~= 'string' or session.sessionkey == ''
        or tonumber(session.premiumuntil) == nil then
        onError(nil, tr('Invalid response received from the login server.'), 0)
        return
    end
    ...
    for index, character in ipairs(decodedCharacters) do
        ...
        if not world then
            onError(nil, tr('The login server returned a character for an unknown world.'), 0)
            return
        end
        characters[index] = {
            name = character.name,
...
function EnterGame.doLogin()
    ...
    if uniqueHost then
        G.host = uniqueHost
        G.port = tonumber(uniqueValues.port)
        clientVersion = tonumber(uniqueValues.protocol)
        httpLogin = uniqueValues.httpLogin == true
    else
        G.host = enterGame:getChildById('serverHostTextEdit'):getText()
        G.port = tonumber(enterGame:getChildById('serverPortTextEdit'):getText())
        clientVersion = tonumber(clientBox:getText())
        httpLogin = enterGame:getChildById('httpLoginBox'):isChecked()
    end
    ...
    local hasHttpEndpoint = type(G.host) == 'string' and G.host:match('^https?://') ~= nil
    if clientVersion >= 1281 and (hasHttpEndpoint or G.port ~= 7171) then
