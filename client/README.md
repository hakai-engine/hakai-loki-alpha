...
## Hakai Engine Client Thor

The Hakai local profile is fixed to protocol `15.25` and authenticates through
`http://127.0.0.1:8088/login`. The HTTP response supplies an opaque session
token and the game-world address; gameplay then connects to Odin on port
`7172`. Canary still contains the legacy account/password path on port `7171`,
but the Hakai `authType = "session"` profile does not support it as an entry
point.

Start the pinned login-server from the sibling `hakai-engine-server-odin`
repository before opening Thor. The endpoint contract is regression-checked by
`tests/lua/test_hakai_login_configuration.lua`.
